...
def generate_verilog_bb( mem ):
    ...
    ########################################
    if num_rport > 0:
      write_init_port_names( V_file, num_rport, 'r' )
    if num_wport > 0:
      write_init_port_names( V_file, num_wport, 'w' )
    if num_rwport > 0:
      write_init_port_names( V_file, num_rwport,'rw' )
    ...
    if num_rport > 0:
      write_ports( V_file, num_rport,'r')
    if num_wport > 0:
      write_ports( V_file, num_wport,'w')
    if num_rwport > 0:
      write_ports( V_file, num_rwport,'rw')
